...
BASELINE_FILE = "change_monitor_baseline.json"
...
def run_scan(target: str):
    xml_file = f"{target}_change_scan.xml"
...
def load_baseline():
    ...
    # ✅ Case 2: old single-host format -> migrate
    ...
    if isinstance(data, dict) and "target" in data and "open_ports" in data:
        ...
        print("[*] Migrated old baseline format to new multi-host format.")
        return migrated

    # ✅ Case 3: unknown format -> reset safely
    print("[!] Baseline format not recognized. Creating a new baseline structure.")
    fresh = {"hosts": {}}
    save_baseline(fresh)
    return fresh
...
if __name__ == "__main__":
    ...
    # If new host -> automatically add it
